...
### Fixed
- **PDF File Path Display**: Removed file path truncation in PDF reports
  - File paths now display in full without being cut off
  - Added text wrapping for long file paths using Paragraph objects
  - Details field also wrapped for better display

## [2.0.122] - 2025-07-25

### Fixed
- **PDF Logo Display**: Fixed logo aspect ratio to maintain square dimensions
  - Logo now properly sized at 1.5x1.5 inches with proportional scaling
  - No longer stretched or distorted
- **PDF Report Table**: Enhanced table to include all required fields
  - Added columns: Status, File Path, Size, Type, Tool, Details, Scan Date
  - Properly sized columns to fit landscape page layout
  - Reduced font sizes for better fit
- **View Button Behavior**: Fixed view button to show PDF by default
  - Clicking view button now opens PDF viewer directly
  - Dropdown still available for choosing between PDF and Table view
- **Main Dashboard View Button**: Fixed view button in main dashboard (index.html)
  - Modified app.viewReport() function to display PDF in iframe instead of JSON
  - Added dropdown menu with options for PDF or JSON view
  - Added hover functionality for dropdown menu

## [2.0.121] - 2025-07-25

### Fixed
- **PDF Logo**: Added missing PixelProbe logo to PDF reports
  - Logo now appears at the top of PDF reports using the existing logo file
  - Properly centered and sized at 2 inches width
- **Report View Dropdown**: Fixed missing dropdown menu CSS preventing PDF/Table view options
  - Added dropdown styles for hover functionality
  - Users can now choose between "View as Table" and "View as PDF" options
- **Scan Progress Loop**: Fixed phase 3 progress bar oscillating between 0 and 1
  - Fixed race condition in scan status endpoint that was resetting phase_current
  - Progress now correctly tracks files processed during scanning phase

### Changed
- Reports are generated as JSON during scans, PDFs are created on-demand when viewed/downloaded
  - This is the intended behavior - not a bug

## [2.0.120] - 2025-07-25

### Added
- **Comprehensive Report Viewer**: Restored full scan report functionality from v2.0.98-2.0.104
  - Added file-by-file scan results display showing status, type, size, and scan date
  - Reports now include up to 1000 scanned files in JSON view and 500 in PDF
  - Dual viewing options: formatted table view and PDF generation
- **Report Filtering**: Added filtering capabilities to reports page
  - Filter by report type (Scan/Cleanup)
  - Filter by scan type (Full/Rescan/Deep Scan/Single File/File Changes)
  - Filter by status (Completed/Error/Running/Cancelled)
- **Export Options**: Added export buttons in report viewer
  - Export as JSON for data analysis
  - Export as PDF for compliance/reporting

### Fixed
- Fixed missing JSON import error preventing reports from loading
- Fixed UI cutoff issues in reports management page with responsive design
- Fixed rescan progress bar to show file names and progress (e.g., "file.mp4 (7/8)")
- Restored table view for reports instead of showing raw JSON
- Fixed PDF generation to include actual scan results

### Changed
- Enhanced scan report generation to include detailed file-by-file results from database
- Improved report viewer with formatted sections for statistics, results, and errors
- PDF reports now include scanned files list with proper formatting

## [2.0.119] - 2025-07-25

### Added
- Added in-browser PDF viewer for reports using PDF.js
  - Reports can now be viewed directly in the application without downloading
  - PDF viewer includes navigation controls (previous/next page)
  - Page number display and keyboard shortcuts (ESC to close)
  - Download button available within the viewer
- Added reportlab to requirements.txt for PDF generation support

### Fixed
- Fixed PDF generation that was broken due to missing reportlab dependency
- Fixed UI layout issues with horizontal scrolling for reports table
- Improved responsive design for better display on various screen sizes

### Changed
- Reports "View" button now opens PDFs in a modal viewer instead of downloading
- Enhanced user experience with integrated PDF viewing capabilities

## [2.0.118] - 2025-07-25

### Fixed
- Enhanced scan progress display with detailed file information
  - Shows current file being scanned with progress (e.g., "7/8")
  - Added phase-specific messages for discovery and database phases
  - Fixed conflicting progress information display
- Improved reports management functionality
  - View button now opens reports in a modal viewer instead of downloading
  - Added dropdown menu for individual report downloads (JSON/PDF options)
  - Bulk download now has dropdown with format options (JSON/ZIP/PDF)
  - Better hover effects and UI consistency for dropdown menus

### Changed
- Progress bar now shows more verbose information during all scan types
- Reports can be viewed in-app without downloading
- Download functionality separated into format-specific options

## [2.0.117] - 2025-07-25

### Changed
- **UI Improvement**: Reports management now opens in a modal popup instead of a separate page
  - Provides better user experience by keeping users on the main dashboard
  - Consistent with other system features (schedules, exclusions, etc.)
  - All reports functionality preserved in the modal interface

## [2.0.116] - 2025-07-25

### Changed
- Updated Docker image version to 2.0.116
- Rebuilt from 2.0.115 branch with all recent UI simplifications
- **UI Improvement**: Reports management now opens in a modal popup instead of a separate page
  - Provides better user experience by keeping users on the main dashboard
  - Consistent with other system features (schedules, exclusions, etc.)
  - All reports functionality preserved in the modal interface
...
### Changed
- **BREAKING**: Removed classic UI template - PixelProbe now uses only the modern UI
- Simplified UI architecture by removing dual-template system
- Removed USE_MODERN_UI environment variable (no longer needed)
- Renamed index_modern.html to index.html as the sole UI template

### Added
- Reports navigation link in the main UI sidebar under System section

## [2.0.114] - 2025-07-24

### Added
- **Major Feature**: Comprehensive report generation for ALL scan types
  - Full scans generate reports with complete statistics
  - Parallel/rescan operations generate reports with scan results
  - File changes scans generate reports with changed files and verification results
  - Single file scans generate reports with individual file results
  - Orphan cleanup operations generate reports with cleanup results
  - All reports saved to `instance/reports/` directory in JSON format
  - Unified report format with scan ID, timestamps, statistics, and results

- **Feature**: Report management API endpoints
  - `GET /api/reports` - List all reports (scan and cleanup) with metadata
  - `GET /api/reports/download-multiple` - Download multiple reports as ZIP or combined PDF
  - `POST /api/reports/delete` - Delete one or more reports
  - `GET /api/cleanup-reports` - List cleanup reports (legacy endpoint)
  - `GET /api/cleanup-reports/<filename>` - Download specific cleanup report

- **Feature**: Multi-format report export
  - Download multiple reports as a single ZIP file
  - Generate combined PDF from multiple reports (requires reportlab)
  - PDF reports include statistics tables and formatted output

### Fixed
- **Critical**: Improved error handling in orphan cleanup deletion phase
  - Fixed "Instance has been deleted" error by using fresh queries instead of refresh
  - Added batch-level error handling to continue processing even if individual batches fail
  - Database rollback on batch errors to maintain consistency

### Enhanced
- **Reporting**: All scan operations now generate detailed reports
  - Reports include scan type, duration, statistics, and results
  - Error reports generated even when scans fail
  - Reports contain comprehensive metadata for analysis
- **Logging**: Added comprehensive logging throughout all operations
  - Log when each orphaned file is found with full path
  - Log batch deletion progress and counts
  - Log report generation status and location
