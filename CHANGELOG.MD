...
- Emails carry `Date` and `Message-ID` headers. `Date` is mandatory per RFC 5322 and `smtplib` does not add one, so without it spam filters penalise the message and strict MTAs can reject it. `Message-ID` is derived from the sender's domain rather than the library default, which would otherwise leak the container hostname.
...
- **A slow sample window no longer discards the windows that did read.** Each window has its own timeout and contributes independently; a partial timeout is surfaced as a warning instead of silently reducing the sample the verdict rests on. The old code also appended a timeout string to `corruption_details`, which the caller only reads when the file is already corrupt, so that message could never reach anyone.
...
- **Two stalled sample windows no longer cost three timeouts.** Whatever makes one window time out (stalled mount, failing sectors, storage slower than realtime decode) applies to the rest, so the second timeout abandons the remaining windows instead of blocking a scan worker for the full 90s to reach the same warning.
